#include "test_cell_in_ecm_contact_via_coupling.hpp"



//Check the contact model contact_node_node_via_coupling by growing a cell inside a static ECM cell. If the 
//the cell is not pushed out of the ECM, then the contact model is working correctly



//---------------------------------------------------------------------------------------------------------
int run_cell_in_ecm_test() noexcept{
    ...
    sim_parameters.input_mesh_path_ =               std::string(PROJECT_SOURCE_DIR) + "/test/test_contact_models/test_contact_node_node_via_coupling/test_cell_in_ecm_contact_via_coupling/cell_in_ecm_geometry.vtk";
    sim_parameters.output_folder_path_ =            std::string(CMAKE_BINARY_DIR) + "/test_cell_in_ecm_contact_via_coupling/";
    ...
    epi_cell_type_ptr->avg_growth_rate_                  = 1e-11;
    ...
    face_type_3.name_                = "ecm";     
    face_type_3.face_type_global_id_ = 3;
    face_type_3.adherence_strength_  = 0;
    face_type_3.repulsion_strength_  = 2e9;
    face_type_3.surface_tension_     = 0; 
    face_type_3.bending_modulus_     = 0;

    //The parameters for the ECM cell type
    auto ecm_cell_type_ptr = std::make_shared<cell_type_parameters>();
    ecm_cell_type_ptr->name_                             = "ecm";     
    ecm_cell_type_ptr->global_type_id_                   = 1; 
    ecm_cell_type_ptr->mass_density_                     = 1e3;
    ecm_cell_type_ptr->bulk_modulus_                     = 1e4;   
    ecm_cell_type_ptr->initial_pressure_                 = 0;
    ecm_cell_type_ptr->max_pressure_                     = 5e2;
    ecm_cell_type_ptr->avg_growth_rate_                  = 0;
    ecm_cell_type_ptr->std_growth_rate_                  = 0;  
    ecm_cell_type_ptr->target_isoperimetric_ratio_       = 150;
    ecm_cell_type_ptr->angle_regularization_factor_      = 0;
    ecm_cell_type_ptr->area_elasticity_modulus_          = 0;  
    ecm_cell_type_ptr->surface_coupling_max_curvature_   = 8e5;
    ecm_cell_type_ptr->avg_division_vol_                 = 1e20;  
    ecm_cell_type_ptr->std_division_vol_                 = 0;   
    ecm_cell_type_ptr->min_vol_                          = 5e-17;   
    ecm_cell_type_ptr->add_face_type(face_type_3);



    try{
        //Call the simulation initializer
        simulation_initializer initializer(sim_parameters, {epi_cell_type_ptr, ecm_cell_type_ptr}, /*verbose = */ true);
